...
// prepare data
//	char *llr_cuda = (char*)malloc(memorySize_llr_cuda*MC);
//	for(int i = 0; i < MC; i++){
//		memcpy((void*)&llr_cuda[i*col*Zc], p_llr, col*Zc*sizeof(char));
//	}
//	for(int j = 0; j < MC; j++){
//		for(int i = 0; i < col*Zc; i++){
//			if(llr_cuda[j*col*Zc +i] != p_llr[i]){
//				printf("%d %d wrong copy\n", j, i);
//				return -1;
//			}
//		}
//	}
